      ...
      this.usersService.login(Email, Password).subscribe({
        next: (response: AuthenticationResponse) => {
          if (response.userID == "admin_id") {
            //admin user
            this.usersService.setAuthStatus(response, response.token, true, response.username);
            this.router.navigate(['admin', 'products']);
          }
          else {
            //normal user
            this.usersService.setAuthStatus(response, response.token, false, response.username);
